...
RECOLOGY_WASTE_SERVICES_SPECIFIC_INSTRUCTIONS = """

RECOLOGY WASTE SERVICES SPECIFIC INSTRUCTIONS:

This template applies to bills with "Recology Waste Services" or "Recology" text.

SECTION 1 - PROVIDER INFORMATION:
**CRITICAL**: Extract Recology Waste Services provider information
- Provider Name: "Recology Waste Services" or "Recology"
...
- Account Number: Look for "Account Number", "Account No", or similar label
  * Extract digits only
- Account Type: Typically "Waste & Recycling" or "Garbage & Recycling"
...
- Bill Type: Identify the type of bill
  ...
  * Look for indicators like "Final Bill", etc.
...
- Extract ALL services (Garbage, Recycling, Organics, Yard Waste, etc.) with complete details
- For EACH service, extract:
  - Service Name: The service type exactly as labeled on the bill
    * Examples: "Garbage", "Recycling", "Organics", "Yard Waste", "Compost"
  ...
  - Line Item Charges: Extract EVERY line item for this service
    * Each line item must include:
      - Category: The charge category/description
        * Examples: "Base Charge", "Container Fee", "Pickup Fee", "Per Pickup Charge"
      - Description: Detailed description (if different from category)
        * May include container size (e.g., "32 Gallon", "64 Gallon", "96 Gallon")
        * May include pickup frequency (e.g., "Weekly", "Bi-weekly", "Monthly")
      - Rate: Rate per unit (if applicable)
      - Amount: Dollar amount for this line item
      - Usage: Number of pickups or containers (if applicable, numeric value)
      - UOM: Unit of measurement (if applicable, e.g., "pickup", "container", "month")

- **IMPORTANT**: Group all services of the same type together into ONE service entry
- The sum of all current_service amounts must equal current_billing
- Note: Waste services typically do NOT have meters or meter readings
...
RECOLOGY_WASTE_SERVICES_JSON_FORMAT = """

Return in this exact JSON format. ALL FIELDS MUST BE INCLUDED for Recology Waste Services bills:
{
  "provider_name": "Recology Waste Services",
      ...
      "line_item_charges": [
        {
          ...
          "meter_number": null,
          "usage": "numeric_value_or_null",
          "uom": "unit_like_pickup_or_container_or_null",
          "previous_reading": null,
...
RECOLOGY_WASTE_SERVICES_PROMPT = f"""{BASE_EXTRACTION_FIELDS}
