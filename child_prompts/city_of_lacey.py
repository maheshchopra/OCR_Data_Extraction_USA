...
CITY_OF_LACEY_SPECIFIC_INSTRUCTIONS = """

CITY OF LACEY SPECIFIC INSTRUCTIONS:

This template applies to bills with "City of Lacey" or "Lacey" text.

SECTION 1 - PROVIDER INFORMATION:
**CRITICAL**: Extract City of Lacey provider information
- Provider Name: "City of Lacey"
...
SECTION 7A - USAGE INFORMATION (CRITICAL):
**CRITICAL**: City of Lacey bills display usage information in multiple formats that must be extracted.

1) USAGE LINES ON THE BILL:
The bill shows usage information like:
- "Usage 17390"
- "1 cf = 7.48 gallons"
- "Your water use for this billing period was 130,077.20 gallons"

2) EXTRACTION RULES:
- Create one `usage_information` entry for EACH distinct usage value you see on the bill.
- For each entry, populate:
  * `usage`: the numeric usage value (string)
  * `uom`: the unit of measurement, e.g. "CF" or "gallons"
  * `multiplier`: the numeric multiplier applied to that usage, as a string

3) EXAMPLES FOR THIS BILL FORMAT:
- For the line "Usage 17390":
  * usage: "17390"
  * uom: "CF"
  * multiplier: "1"

- For the line "Your water use for this billing period was 130,077.20 gallons":
  * usage: "130077.20"
  * uom: "gallons"
  * multiplier: "7.48"   (because 1 CF = 7.48 gallons)

4) IMPORTANT:
- Do NOT include `conversion_factor`, `from_unit`, or `to_unit` fields.
- Only return `usage`, `uom`, and `multiplier` for each entry.
- If a multiplier is not shown or not implied, set `multiplier` to null.
- The primary CF usage should be the first entry; converted gallons usage should be a separate entry.
...
CITY_OF_LACEY_JSON_FORMAT = """

Return in this exact JSON format. ALL FIELDS MUST BE INCLUDED for City of Lacey bills:
{
  "provider_name": "City of Lacey",
  ...
  "usage_information": [
    {
      "usage": "numeric_usage_value",
      "uom": "unit_like_CF_or_gallons",
      "multiplier": "numeric_multiplier_like_1_or_7.48_or_null"
    }
  ],
...
CITY_OF_LACEY_PROMPT = f"""{BASE_EXTRACTION_FIELDS}
