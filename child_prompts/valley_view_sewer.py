...
VALLEY_VIEW_SEWER_DISTRICT_SPECIFIC_INSTRUCTIONS = """

VALLEY VIEW SEWER DISTRICT SPECIFIC INSTRUCTIONS:

This template applies to bills with "Valley View Sewer District" text.

SECTION 1 - PROVIDER INFORMATION:
- Provider Name: "Valley View Sewer District"
- Extract provider website, customer service phone, email, address, and remit address.

SECTION 2 - ACCOUNT INFORMATION:
- Account number, account type (Sewer), bill number/type, customer name, service and billing addresses.
...
- Populate late_fee_info structure with late fee details, rates, totals.

SECTION 7 - SERVICE CHARGES:
- For each sewer service (base charge, ERU charge, flow charge, surcharge, etc.) gather:
  * service_name, current_service, service_from/to dates.
  * Each line_item_charges entry must include category, description, rate, amount, usage, uom.
  * CRITICAL: For line items showing multiple units (e.g., "11 Units @", "95 Units @"), extract the TOTAL amount charged for all those units, NOT the per-unit rate.
    - Example: If the bill shows "11 Units @ $46.63", the amount should be $512.93 (11 × $46.63), not $46.63.
    - The "amount" field must represent the total charge for that line item.
    - The "rate" field (if shown) should contain the per-unit rate.
  * The sum of all line_item_charges amounts must equal the current_service amount for that service.
- Group charges with same service name together.
- The sum of all service current_service amounts must equal current_billing.

SECTION 8 - TAXES:
- Separate per_service_taxes and global_taxes with name, rate, amount.

SECTION 9 - ADJUSTMENTS:
- Capture each adjustment with category and signed amount.

"""

VALLEY_VIEW_SEWER_DISTRICT_JSON_FORMAT = """

{
  "provider_name": "Valley View Sewer District",
...
VALLEY_VIEW_SEWER_DISTRICT_PROMPT = f"""{BASE_EXTRACTION_FIELDS}
