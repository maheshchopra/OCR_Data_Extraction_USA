...
CEDAR_GROVE_ORGANICS_RECYCLING_SPECIFIC_INSTRUCTIONS = """

CEDAR GROVE ORGANICS RECYCLING SPECIFIC INSTRUCTIONS:

This template applies to bills with "Cedar Grove Organics Recycling" or "Cedar Grove" text.

SECTION 1 - PROVIDER INFORMATION:
**CRITICAL**: Extract Cedar Grove Organics Recycling provider information
- Provider Name: "Cedar Grove Organics Recycling" or "Cedar Grove"
...
- Account Number: Look for "Account Number", "Account No", or similar label
  * Extract digits only
- Account Type: Typically "Organics Recycling" or "Recycling"
...
- Service Address: Physical location where recycling services are provided
...
SECTION 7 - SERVICE CHARGES (MOST CRITICAL):
- Extract ALL services (Organics Recycling, Compost Pickup, Yard Waste, etc.) with complete details
- For EACH service, extract:
  - Service Name: The service type exactly as labeled on the bill
    * Common services: Organics Recycling, Compost Service, Yard Waste Collection, etc.
  ...
  - Line Item Charges: Extract EVERY line item for this service
    * Each line item must include:
      - Category: The charge category/description
        * Examples: Container rental, Pickup fee, Service charge, etc.
      - Description: Detailed description (if different from category)
        * May include container size, pickup frequency, service details
      - Rate: Rate per unit (if applicable)
        * May be per container, per pickup, per month, etc.
      - Amount: Dollar amount for this line item
      - Quantity: Number of containers or pickups (if shown)
      - Service Period: Date range for this specific charge (if different from service dates)

- **IMPORTANT**: Extract all service charges with detailed descriptions
...
SPECIAL NOTES FOR CEDAR GROVE ORGANICS RECYCLING BILLS:
- Cedar Grove bills typically provide organics recycling and compost services
- Service charges may include container rentals, pickup fees, and service charges
- Container sizes and pickup frequencies are important details to extract
...
- Service charges are typically flat fees or per-container/per-pickup rates
- No meter readings are typically involved (unlike water/wastewater bills)

"""

CEDAR_GROVE_ORGANICS_RECYCLING_JSON_FORMAT = """

Return in this exact JSON format. ALL FIELDS MUST BE INCLUDED for Cedar Grove Organics Recycling bills:
{
  "provider_name": "Cedar Grove Organics Recycling",
  ...
  "account_number": "numeric_digits_only_or_null",
  "account_type": "Organics Recycling",
  ...
  "service_types": [
    {
      ...
      "line_item_charges": [
        {
          "category": "category_description_as_shown_on_bill",
          "description": "detailed_description_with_container_size_frequency_or_null",
          "rate": "rate_per_unit_or_null",
          "amount": "dollar_amount",
          "meter_number": null,
          "usage": null,
...
CEDAR_GROVE_ORGANICS_RECYCLING_PROMPT = f"""{BASE_EXTRACTION_FIELDS}
