...
class ChargesLevelData(BaseModel):
    """Individual service line items from the charges section."""
    ...
    estimated_landfill_diversion: Optional[str] = None
...
class TotalAmountValidation(BaseModel):
    """Used to validate the sum of (previous balance, current billing, adjustments, minus payments) with the total amount due."""
...
class CedarGroveBillExtract(BaseModel):
    """Complete Cedar Grove Organics Recycling LLC bill extraction."""
