...
class StatementLevelData(BaseModel):
    """Statement-level financial information for Seattle City Light commercial bills."""
...
class MeterLevelData(BaseModel):
    ...
    For commercial bills, this includes multiple service categories (KWH, KW, KVRH)
    per meter with separate readings and power factor calculations.
    """
...
class AccountLevelData(BaseModel):
    """Account-level information for Seattle City Light commercial bills."""
...
class SeattleCityLightCommercialBillExtract(BaseModel):
    """Complete Seattle City Light commercial bill extraction.
    
    This model is for the complex bill format with multiple service categories
    (KWH, KW, KVRH) per meter, power factor penalties, and detailed consumption tracking.
    """
